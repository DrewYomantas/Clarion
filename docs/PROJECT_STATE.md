...
5. Honest next move:
   ...
   - current live acquisition status:
     ...
     - Wave80 scout queue now includes the six documented Pass 36 Google Maps `2-star` dead lanes, eight more same-pass Pass 37 Google Maps surfacing failures, and the captured fallback sources
   ...
   - next live Wave80 pass should:
     ...
     - keep pushing honest `2-star` growth from new-state practice lanes, but activate the controlled fallback rule once six Google Maps surfacing failures are documented in the same pass
     ...
     - if the pass re-earns fallback but still cannot surface a new Google Maps `2-star`, stop honestly at the fallback cap instead of padding the batch
