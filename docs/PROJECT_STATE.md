...
Latest commit to deploy after reconnect: latest `main` head. Current auth blocker stack: live auth/session failed for fresh verified users because Postgres-backed session loads were returning tuple rows into `load_user`, and the service is still unsafe when Redis is unavailable with multiple workers. Repo fixes are in `0f8c954` plus the earlier `/login` handoff fix `adc22c8`.
