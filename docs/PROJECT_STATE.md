...
- The current blocker is no longer filing-delay truth design; it is whether the remaining shortlisted trust pressure on `newfrontier_vlopez` should be resolved before any future retry broader than the two ready rows.
...
### Current Next Pass
5. Honest next move:
   - latest completed milestone is Pass 53 (`2026-04-08`)
   ...
   - the shortlist truth review is also complete:
     - ready for very selective promotion retry: `ryangarry_noellevitzthum`, `fulton_kellieprenslow`
     - keep on shortlist but not ready: `newfrontier_vlopez`
   - next useful step is a very selective promotion retry on the ready subset only
