...
**Broad outcome_satisfaction benchmark-design cleanup is now the active repo priority.**
...
- The `outcome_satisfaction` bucket is not yet clean enough for a larger engine pass because it still mixes explicit result language with generic praise, speculative-result language, and mixed service-complaint rows.
...
### Current Next Pass
5. Honest next move:
   - latest completed milestone is Pass 57 (`2026-04-08`)
   ...
   - next useful step is one truth-shaping pass on the broad `outcome_satisfaction` bucket before any wider engine pass or future promotion widening
