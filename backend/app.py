...
from services.csv_upload_parser import parse_csv_upload_rows
...
def _parse_csv_upload_rows(upload_file, access_type):
    return parse_csv_upload_rows(
        upload_file,
        access_type,
        max_csv_rows=MAX_CSV_ROWS,
        max_csv_field_length=MAX_CSV_FIELD_LENGTH,
        max_review_text_length=MAX_REVIEW_TEXT_LENGTH,
        free_plan_max_reviews=FREE_PLAN_MAX_REVIEWS_PER_REPORT,
        onetime_max_reviews=ONETIME_MAX_REVIEWS_PER_REPORT,
    )
...
def api_billing_checkout():
    return billing_routes.api_billing_checkout()

@app.route('/api/billing/checkout/finalize', methods=['POST'])
@limiter.limit('30 per hour')
@login_required
def api_billing_checkout_finalize():
    return billing_routes.api_billing_checkout_finalize()

@app.route('/api/auth/login', methods=['POST'])
@csrf.exempt
@limiter.limit('5 per minute')
def api_login():
    return auth_routes.api_login()
...
def api_verify_two_factor():
    return auth_routes.api_verify_two_factor()

@app.route('/api/auth/2fa/enable', methods=['POST'])
@login_required
def api_enable_two_factor():
    return auth_routes.api_enable_two_factor()

@app.route('/api/auth/2fa/disable', methods=['POST'])
@login_required
def api_disable_two_factor():
    return auth_routes.api_disable_two_factor()

@app.route('/api/auth/register', methods=['POST'])
@csrf.exempt
@limiter.limit('5 per minute')
def api_register():
    return auth_routes.api_register()

@app.route('/api/auth/me', methods=['GET'])
def api_get_me():
    return auth_routes.api_get_me()

@app.route('/api/auth/verify-email/<token>', methods=['GET'])
@csrf.exempt
@limiter.limit('30 per hour')
def api_verify_email(token):
    return auth_routes.api_verify_email(token)
...
def api_account_branding_get():
    return account_routes.api_account_branding_get()
...
def api_account_branding_logo_get():
    return account_routes.api_account_branding_logo_get()

@app.route('/api/account/branding/logo', methods=['POST'])
@login_required
def api_account_branding_logo_put():
    return account_routes.api_account_branding_logo_put()

@app.route('/api/account/branding/logo', methods=['DELETE'])
@login_required
def api_account_branding_logo_delete():
    return account_routes.api_account_branding_logo_delete()

@app.route('/api/account/branding/theme', methods=['PUT'])
@login_required
def api_account_branding_theme_put():
    return account_routes.api_account_branding_theme_put()

@app.route('/api/team/invite', methods=['POST'])
@login_required
def api_team_invite():
    return team_routes.api_team_invite()

def _api_team_invite_disabled_legacy():
    """Legacy disabled stub retained as a compatibility reference only."""
...
def api_team_accept():
    return team_routes.api_team_accept()
...
def api_support_ticket_create():
    return support_routes.api_support_ticket_create()

@app.route('/api/support/tickets', methods=['GET'])
@login_required
def api_support_tickets_list():
    return support_routes.api_support_tickets_list()

@app.route('/api/support/tickets/<int:ticket_id>', methods=['PATCH'])
@login_required
def api_support_ticket_update(ticket_id):
    return support_routes.api_support_ticket_update(ticket_id)
